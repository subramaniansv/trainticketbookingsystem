...
public class TatkalBookingController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (seatCount > 7) {
			sendResponse(new ApiResponse(false, " ticket not  booked seatc ount must be less than 7", null, 200),
					response);
			return;
		}
		LocalDate bookingDate = bookingDto.getBookingDate().toLocalDate();

		if (bookingDate.isAfter(LocalDate.now().plusDays(1))) {
			sendResponse(
					new ApiResponse(false, "Tatkal opens only a day before", null, 200),
					response);
			return;
		}
		BookingDto book = service.tatkalTicketBooker(bookingDto);
